...
[![shadowsocks-rust](https://github.com/zdevcs/docker-install/actions/workflows/shadowsocks_rust-publish.yml/badge.svg)](https://github.com/zdevcs/docker-install/actions/workflows/shadowsocks_rust-publish.yml)
...
```bash
$ docker pull pkg.zdev.me/shadowsocks-rust:latest
```
...
```bash
$ docker build --tab shadowsocks-rust . --file Dockerfile
```
### 为Arm64以及x64平台编译
- Install buildx
``` bash
...
- Update Dockerfile
``` bash
sed -i 's/FROM[[:space:]]*\(--platform=${TARGETPLATFORM}[[:space:]]*\)\?/FROM --platform=${TARGETPLATFORM} /g' Dockerfile
```
- Compile for Arm64 and x64 platforms
``` bash
docker run --rm --privileged tonistiigi/binfmt:master --install all
docker buildx create --use --name build --node build --driver-opt network=host
docker buildx build --tab shadowsocks-rust . --file Dockerfile --platform linux/amd64,linux/arm64
```
